...
export default function OfficialDashboard() {
    ...
    const handleApprove = async (cert: Certificate) => {
        try {
            const response = await axios.put(`${API_URL}/higher-officer/certificate/status/update/${cert._id}`, {
                status: 'approved',
                remarks: rejectReason || t('remark_higher_approved') || 'Verified and approved by Higher Official'
            }, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
    ...
    const pendingCerts = filteredBySearch.filter(c => getHigherAuthorityStatus(c) === 'pending');
    const approvedCerts = filteredBySearch.filter(c => getHigherAuthorityStatus(c) === 'approved');
    const rejectedCerts = filteredBySearch.filter(c => getHigherAuthorityStatus(c) === 'rejected');
            ...
            <main className="flex-1 container mx-auto px-4 py-8">
                ...
                >
                    <div>
                        ...
                        <p className="text-muted-foreground">{t('officer_higher_desc')}</p>
                    </div>
                            ...
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-border">
                                    {displayCerts.map((cert) => {
                                        const higherStatus = getHigherAuthorityStatus(cert);
                                        return (
                                            <tr key={cert._id} className="hover:bg-muted/30 transition-colors">
                                                ...
                                                <td className="px-4 py-4 whitespace-nowrap">
                                                    {/* Using the standard status badge since Higher controls final status */}
                                                ...
                                                <td className="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    ...
                                                    >
                                                        ...
                                                        {higherStatus === 'pending' ? t('table_review') : t('table_view')}
                                                    </Button>
                                                </td>
                                            </tr>
                                        );
            ...
            <Dialog open={!!selectedCert} onOpenChange={(open) => {
                ...
                <DialogContent className="max-w-6xl max-h-[90vh] overflow-hidden flex flex-col p-0">
                    ...
                    <div className="px-6 py-4 border-b flex justify-between items-center bg-background/95 backdrop-blur z-10 shrink-0">
                      <div>
                        <DialogTitle className="font-heading text-xl text-indigo-700 dark:text-indigo-400 m-0">
                          {t('modal_review_cert')}
                        </DialogTitle>
                        {selectedCert && (
                    ...
                    {selectedCert && (
                      <div className="flex-1 overflow-hidden grid grid-cols-1 lg:grid-cols-2">
                        ...
                        <div className="overflow-y-auto p-6 lg:p-8 space-y-8 bg-background">
                            ...
                            <div className="grid grid-cols-2 gap-y-6 gap-x-4 text-sm bg-indigo-500/5 p-5 rounded-xl border border-indigo-500/20 shadow-sm relative overflow-hidden">
                                ...
                                <div>
                                    ...
                                    <StatusBadge status={getHigherAuthorityStatus(selectedCert)} />
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="space-y-0">
                                  ...
                                  {selectedCert.approvalHistory?.length > 0 && (
                                     <div className="border-l-2 border-indigo-500/30 ml-2 pl-4 py-2 relative">
                                        <div className="absolute -left-[5px] top-4 bg-indigo-500 w-2 h-2 rounded-full" />
                                  ...
                                  {/* Mid */}
                                  {selectedCert.seniorapprovalhistory?.length > 0 && (
                                     <div className="border-l-2 border-indigo-500/30 ml-2 pl-4 py-2 relative">
                                        <div className="absolute -left-[5px] top-4 bg-indigo-500 w-2 h-2 rounded-full" />
                                        <p className="text-xs font-bold text-foreground">{t('modal_mid_level')}</p>
                                        {selectedCert.seniorapprovalhistory.map((h, i) => (
                                          <div key={i} className="mt-1 bg-muted/30 p-2 rounded text-sm">
                                            <div className="flex items-center justify-between mb-1">
                                              <StatusBadge status={h.action} />
                                              <span className="text-[10px] text-muted-foreground">{new Date(h.timestamp).toLocaleString()}</span>
                                  ...
                                  {/* Higher (Self) */}
                                   {(selectedCert.higherapprovalhistory?.length || 0) > 0 && (
                                     <div className="border-l-2 border-indigo-500/30 ml-2 pl-4 py-2 relative">
                                        <div className="absolute -left-[5px] top-4 bg-indigo-500 w-2 h-2 rounded-full ring-2 ring-background" />
                                        <p className="text-xs font-bold text-foreground">{t('modal_higher_level') || 'Higher Authority'}</p>
                                        {selectedCert.higherapprovalhistory.map((h, i) => (
                                          <div key={i} className="mt-1 bg-muted/30 p-2 rounded text-sm">
                                            <div className="flex items-center justify-between mb-1">
                                              <StatusBadge status={h.action} />
                                              <span className="text-[10px] text-muted-foreground">{new Date(h.timestamp).toLocaleString()}</span>
                            ...
                            {getHigherAuthorityStatus(selectedCert) === 'pending' && (
                                <div className="pt-6 border-t mt-auto space-y-4">
                                    ...
                                    <div className="flex gap-3 pt-2">
                                        ...
                                        <Button size="lg" className="flex-1 bg-indigo-600 hover:bg-indigo-700" onClick={() => handleApprove(selectedCert)}>
