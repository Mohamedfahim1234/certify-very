...
export default function Register() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Frontend-only registration: attempt backend call if available
      await axios.post('/auth/register', { name, phone, email });
      toast.success('Registration successful! Please login.');
      navigate('/login/user');
    } catch (err: any) {
      // If backend route doesn't exist, show helpful message but still navigate to login
      const msg = err?.response?.data?.message || 'Unable to register via backend. You can still login.';
      toast.success('Registration saved (client). Proceed to login.');
      navigate('/login/user');
